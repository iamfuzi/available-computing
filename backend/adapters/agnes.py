...
def _infer_category(model_id: str) -> str:
    ...
    # Text-to-image / text-to-video generation models do not accept chat
    # requests; classify them so the chat-prober skips them (and so they stay
    # out of /v1/models). Only the chat-capable flash model is whitelisted.
    if "video" in lower:
        return "video"
    if "image" in lower:
        return "image"
...
class AgnesAdapter(ProviderAdapter):
    ...
    def detect_free_from_api(self, model: ModelInfo) -> Optional[dict]:
        # Agnes doesn't expose pricing in the models API; rely on whitelist.
        return None
