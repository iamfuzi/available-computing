    ...
    <div className="max-w-5xl mx-auto px-4 py-6 space-y-5">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        <div className="bg-white border border-gray-200 rounded-xl p-3">
          <div className="text-xs text-gray-400">可继续审核</div>
          <div className="text-xl font-semibold mt-1">{candidates.filter(c => c.status === 'pending' && c.admission_status === 'review_required').length}</div>
        </div>
        <div className="bg-white border border-gray-200 rounded-xl p-3">
          <div className="text-xs text-gray-400">OpenAI 兼容候选</div>
          <div className="text-xl font-semibold mt-1">{candidates.filter(c => c.compatibility === 'openai_compatible').length}</div>
        </div>
        <div className="bg-white border border-gray-200 rounded-xl p-3">
          <div className="text-xs text-gray-400">准入排除</div>
          <div className="text-xl font-semibold mt-1 text-red-600">{candidates.filter(c => c.admission_status === 'excluded').length}</div>
        </div>
        <div className="bg-white border border-gray-200 rounded-xl p-3">
          <div className="text-xs text-gray-400">抓取来源</div>
